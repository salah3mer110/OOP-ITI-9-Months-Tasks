...
int  takeChoices(int,span<emp>,int &);
void displayMenu();
emp addEmployee(int);
...
int  takeChoices(int choice, span<emp> employees,int& Empcount) {
    int key = _getch();
    if (key == 27) {//exit
        system("cls");
        cout << "bye bye :)\n";
    ...
    case 13: //enter
        ...
        if (choice == 0) {
            system("cls");
            employees[Empcount]= addEmployee(Empcount);
            Empcount++;
            returnMenu();
        }
        if (choice == 1 ) {
